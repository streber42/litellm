...
class TestValidateEnvironment:
    ...
    def test_module_var_takes_precedence_over_env(self, monkeypatch):
        ...
        litellm.opencode_zen_api_key = "sk-module"
        try:
            headers: dict = {}
            result = self.cfg.validate_environment(
                headers=headers,
...
class TestMockedCompletion:
    ...
    def test_go_dispatch_custom_llm_provider(self, respx_mock):
    ...
    def test_unknown_model_routes_to_chat_arm(self, respx_mock):
    ...
    def test_upstream_error_surfaces_as_connection_error(self, respx_mock):
    ...
    def test_bearer_auth_from_surface_key(self, respx_mock, monkeypatch):
        ...
        litellm.disable_aiohttp_transport = True
        respx_mock.post("https://opencode.ai/zen/v1/chat/completions").mock(
            return_value=Response(200, json=_make_response("gpt-5.1", "ok"))
        )
...
class TestStreaming:
    ...
    def test_async_streaming_handler_type(self):
        """Async streaming returns the OpenAI handler instance."""
        cfg = OpenCodeConfig(surface="zen")

        import httpx

        # Pass a real async-iterable response so aiter_lines() is exercised.
        httpx_response = httpx.Response(
            200,
            headers={"transfer-encoding": "chunked"},
            stream=httpx.ByteStream(b'data: {"choices": [{"delta": {"content": "hi"}}]}'),
        )
        handler = cfg.get_async_streaming_response_iterator(
            model="grok-4.5",
