...
class OpenCodeConfig(OpenAIGPTConfig):
    ...
    def validate_environment(
        ...
        if key is None:  # rebind-ok: iterative fallback through key sources
            key = getattr(litellm, f"opencode_{self.surface}_api_key", None)  # rebind-ok: iterative fallback through key sources

        if key is None:  # rebind-ok: iterative fallback through key sources
            key = get_secret_str(f"OPENCODE_{surface_upper}_API_KEY")  # rebind-ok: iterative fallback through key sources
