...
def _complete_opencode(
    ...
    api_base = (  # rebind-ok: resolve base from module/env fallbacks
        api_base
    ...
    _headers: Final = {**base_headers, "Authorization": f"Bearer {api_key}"} if api_key is not None else base_headers  # mutable-ok: dict literal for request headers
    ...
    if is_messages_model(surface, model):
        ...
        provider_config: Final = OpenCodeMessagesConfig(surface=surface)
        ...
        _messages_api_base = api_base  # rebind-ok: strip trailing /v1 suffix in loop
        ...
        messages_handler_coro: Final = base_llm_http_handler.async_anthropic_messages_handler(
            model=model,
            messages=messages,
        ...
        response = asyncio.run(messages_handler_coro)  # rebind-ok: chat arm rebinds below
        logging.post_call(input=messages, api_key=api_key, original_response=response)
        return response
